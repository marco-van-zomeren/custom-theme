...
jQuery(window).load(function () {
    ...
    jQuery('.page-container').css('background-color', 'rgb(' + color + ')');
    jQuery('#container').css('background-color', 'rgb(' + color + ')');
...
function lightOrDark(color) {
    ...
    if (color.match(/^rgb/)) {
        ...
        color = +("0x" + color.slice(1).replace(
            color.length < 5 && /./g, 'jQuery&jQuery&'
        ));
